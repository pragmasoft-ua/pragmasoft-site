<!-- Footer -->
<div class="footer">
  <div class="container">
    <div class="row">
      <div class="fot-img-sm"><img src="${app}res/img/01m_txt_2.png"></div>
      <div class="col-md-6 col-sm-6 col-xs-12 col-lg-6">
        <img src="${app}res/img/pragmasoft.png" alt="Pragmasoft">
        <p>For any questions, please contact<br/>us at <a href="mailto:info@pragmasoft.com.ua">info@pragmasoft.com.ua</a></p>
      </div>
      <div class="col-md-6 col-sm-6 col-xs-12 col-lg-6">
        <p class="soc">We are in social networks:</p>
        <div class="s-icon-block">
          <a class="vk" href="http://vk.com/club32131862" target="_blank"></a>
          <a class="fb" href="https://www.facebook.com/pages/Pragmasoft/340275132655051" target="_blank"></a>
          <a class="gp" href="https://plus.google.com/u/0/101843999873125528372" target="_blank"></a>
          <a href="#" data-toggle="modal" data-target="#myModal">
            <div class="mail"></div>
          </a>
        </div>
      </div>
    </div>
  </div>
</div>

<!--Copyright  -->      
<div class="copyright">
  <div class="container">
    <!-- Example row of columns -->
    <div class="row">
      <div class="col-md-6 col-sm-6 col-xs-12 col-lg-6">
        <p>© 2014 Pragmasoft. All rights reserved</p>
      </div>
      <div class="col-md-6 col-sm-6 col-xs-12 col-lg-6">
        <a href="mailto:info@pragmasoft.com.ua" data-toggle="modal" data-target="#myModal">Contact Us</a>
      </div>
    </div>
  </div>
</div>