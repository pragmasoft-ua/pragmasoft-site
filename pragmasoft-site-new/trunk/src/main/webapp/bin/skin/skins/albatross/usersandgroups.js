...
var ASSTable = Class.create({
  ...
  displayRows: function(offset, limit)
  {
    ...
    var msg = "?? <strong>" + off + "</strong> To <strong>" + f + "</strong> out of <strong>" + this.totalRows + "</strong>";
    var msg = msg.toLowerCase();
    $('showLimits').innerHTML = "Displaying rows " + msg;
...
MSCheckbox = Class.create({
  ...
  initialize: function(domNode, right, saveUrl, defaultState, table, idx)
  {
    ...
    this.images = ["/skins/albatross/icons/rights%2Dmanager/none.png","/skins/albatross/icons/rights%2Dmanager/allow.png","/skins/albatross/icons/rights%2Dmanager/deny1.png"];
    this.labels = ['','',''];
  ...
  createClickHandler: function(self)
  {
    return function() {
      ...
      if(self.currentUorG == window.currentUser) {
	       if (nxtst == 2) {
	       	 var denymessage = "You are about to deny the __right__ right for yourself. Continue?".replace('__right__', self.right);
           if (!confirm(denymessage)) {
           	  var clearmessage = "Would you like to clear the __right__ right for yourself instead?".replace('__right__', self.right);
	          ...
	          var clearmessage = "You are about to clear the __right__ right for yourself. Continue?".replace('__right__', self.right);
	          if (!confirm(clearmessage)) {
	           return;
	          }
	       }
      ...
      // 2. The current user is clearing / denying any rights for a group he belongs to.
      	...
      	if (nxtst == 2) {
           var denymessage = "You are about to deny the __right__ right for  __name__. This implies denying your own __right__ right, if you are part of this group. Continue?".replace(/__right__/g, self.right);
           denymessage = denymessage.replace('__name__', self.currentUorG);
           if (!confirm(denymessage)) {
           	var clearmessage = "Would you like to clear the __right__ right for __name__ instead? This implies clearing your own __right__ right, if you are part of this group. Continue?".replace(/__right__/g, self.right);
            ...
            var clearmessage = "You are about to clear the __right__ right for __name__. This implies clearing your own __right__ right, if you are part of this group. Continue?".replace(/__right__/g, self.right);
            clearmessage = clearmessage.replace('__name__', self.currentUorG);
            if (!confirm(clearmessage)) {
             return;
            }
      ...
      // 3. The current user is is clearing / denying admin right for any user / group.
      	...
      	if (nxtst == 2) {
          var denymessage = "You are about to deny the __right__ right for __name__. Continue?".replace('__right__', self.right);
          denymessage = denymessage.replace('__name__', self.currentUorG);
          if (!confirm(denymessage)) {
            return;
          }
          ...
          var clearmessage = "You are about to clear the __right__ right for __name__. Continue?".replace('__right__', self.right);
          clearmessage = clearmessage.replace('__name__', self.currentUorG);
          if (!confirm(clearmessage)) {
            return;
          }
      ...
      self.req = new Ajax.Request(url,
      {
        ...
        onSuccess: function(transport) {
            ...
            self.next();
          	...
          	// that action
            alert("An exception occured while trying to save the current modifications. Please check if you have the proper rights to perform these modifications.");
...
function displayUsers(row, i, table)
{
  ...
  if (wikiname == "local") {
    ...
    edit.src = '/skins/albatross/icons/rights%2Dmanager/edit.png';
    edit.title = "??????";
    ...
    if(row.grayed == "true") {
      del.src = '/skins/albatross/icons/rights%2Dmanager/clearg.png';
      del.className = 'icon-manageg';
    } else {
      del.src = '/skins/albatross/icons/rights%2Dmanager/clear.png';
...
{
  return function() { 
      var url = '/bin/save/XWiki/XWikiPreferences';
      var pivot = self;
      if(self.getAttribute('alt') == "yes")
      {
        if(self.id.indexOf('view') > 0)
           new Ajax.Request(url, {method: 'post', parameters: {"XWiki.XWikiPreferences_0_authenticate_view" : "0"}, onSuccess: function() { pivot.alt = "no"; pivot.src = "/skins/albatross/icons/rights%2Dmanager/none.png"; }  }); 
        else
           new Ajax.Request(url, {method: 'post', parameters: {"XWiki.XWikiPreferences_0_authenticate_edit" : "0"}, onSuccess: function() { pivot.alt = "no"; pivot.src = "/skins/albatross/icons/rights%2Dmanager/none.png"; }  });
      }
      else {
           if(self.id.indexOf('view') > 0)
             new Ajax.Request(url, {method: 'post', parameters: {"XWiki.XWikiPreferences_0_authenticate_view" : "1"}, onSuccess: function() { pivot.alt = "yes"; pivot.src = "/skins/albatross/icons/rights%2Dmanager/allow%2Dblack.png"; }  }); 
           else
             new Ajax.Request(url, {method: 'post', parameters: {"XWiki.XWikiPreferences_0_authenticate_edit" : "1"}, onSuccess: function() { pivot.alt = "yes"; pivot.src = "/skins/albatross/icons/rights%2Dmanager/allow%2Dblack.png"; }  });
      }  	
  }
}
